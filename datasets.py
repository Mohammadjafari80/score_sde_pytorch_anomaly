...
class MVTecDatasetScores(Dataset):
    def __init__(self, root, category, transform=None, target_transform=None, train=True, normal=True, scores=None):
        self.transform = transform
        if train:
            self.image_files = glob(
                os.path.join(root, category, "train", "good", "*.png")
    ...
    def __getitem__(self, index):
        ...
        return image, target, self.scores[index]
        # , self.scores[index]
